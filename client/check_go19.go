...
func (c *Client) Check(key []byte) (CheckStatus, error) {
	...
	var (
		...
		// is corrupted
		cStatus     = make(chan pb.CheckResponse_Status, len(idsPerShard))
		wg          sync.WaitGroup
	...
	for shard, ids := range idsPerShard {
		go func(ctx context.Context, shard string, ids [][]byte, cStatus chan<- pb.CheckResponse_Status, cErr chan<- error) {
			defer wg.Done()
